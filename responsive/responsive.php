...
/**
 * A plugin that makes widgets responsive and gives you nifty responsive column shortcodes.
 *
 * @author Greg Priday <greg@siteorigin.com>
 * @copyright Copyright (c) 2012, SiteOrigin
 * @license GPL <siteorigin.com/gpl>
 */
class OriginResponsiveController extends Origin_Controller {
	private $sidebars;
	...
	function __construct(){
		...
		add_action('register_sidebar', array($this, 'action_register_sidebar'));
		add_action('wp_print_styles', array($this, 'action_wp_print_sidebar_styles'));
	}
	...
	function action_register_sidebar($sidebar){
	...
	function action_wp_print_sidebar_styles(){
		...
		foreach($this->sidebars as $id => $sidebar){
			...
			foreach($responsive_resolutions as $resolution){
				...
				$columns = self::get_nearest_res_value($responsive, $resolution);
...
OriginResponsiveController::single();